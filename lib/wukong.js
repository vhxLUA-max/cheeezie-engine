...
// chess engine object
var Wukong = function (boardSize, lightSquare, darkSquare, selectColor) {
  /****************************\
   ============================
  ...
  */
  ...
  function searchPosition(depth, side = "white") {
    let start = Date.now();
    let score = 0;
    let lastBestMove = 0;
    ...
    for (let index = 0; index < moves.length; index++) {
      ...
      if (validMove) {
        ...
        if (typeof document != "undefined") {
          ...
          moveStack[moveStack.length - 1].piece =
            board[getMoveTarget(validMove)];
          moveStack[moveStack.length - 1].inCheck = isSquareAttacked(
            kingSquare[side],
            side ^ 1
          )
            ? 1
